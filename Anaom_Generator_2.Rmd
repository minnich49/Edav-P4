...
#df_test$Created.Date2 = as.Date(as.character(df_test$Created.Date),format ="%m/%d/%Y %I:%M:%S %p")
df_test$Created.Date2 = as.POSIXct(df_test$Created.Date,format ="%m/%d/%Y %I:%M:%S %p")
df_test$Day = strftime(df_test$Created.Date2, "%d")
df_test$Month = strftime(df_test$Created.Date2, "%m")
df_test$Year = strftime(df_test$Created.Date2, "%Y")

df2 = group_by(df_test,Day,Month,Year)
df3 = summarise(df2, count = n())
...
df4 = df3[,c("Date","count")]
df4 = df4[complete.cases(df4),]
df4[order(df4$count),]

res = AnomalyDetectionTs(df4, max_anoms=0.05, direction='both', plot=TRUE,piecewise_median_period_weeks = 2)
res
res$anoms[order(res$anoms$anoms),]

test = filter(df_test,Year == 2015 & Month == "01" & Day == "08")
df2 = group_by(test,Day,Month,Year)
df3 = summarise(df2, count = n())

write.csv(test,"Jan_08_2015.csv")
